...
return new class extends Migration
{
    ...
    {
        Schema::create('tabungan_detils', function (Blueprint $table) {
            $table->id();
            $table->string('nama', 2048);
            $table->timestamp('tanggal')->nullable();
            $table->enum('isambil', ['Y', 'N']);
            $table->integer('jumlah', false, true);
            $table->foreignId('tabungan_id')->constrained()->cascadeOnUpdate()->restrictOnDelete();
    ...
    {
        Schema::dropIfExists('tabungan_detils');
    }
};
